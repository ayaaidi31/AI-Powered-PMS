      ...
      <div className="min-h-screen bg-background flex items-center justify-center">
        <Card className="w-full max-w-md">
          <CardContent className="pt-6 text-center">
            ...
            <h2 className="text-xl font-semibold text-foreground mb-2">{t("patientRecords.reportNotFound")}</h2>
            <p className="text-muted-foreground mb-6">{t("patientRecords.reportNotFoundDesc")}</p>
            <Link href="/patient/records"><Button>{t("patientRecords.backToRecords")}</Button></Link>
          </CardContent>
        </Card>
      </div>
    )
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <h1 className="text-lg font-semibold text-foreground">{report.diagnosis ?? t("patientRecords.reportFallbackTitle")}</h1>
              <p className="text-sm text-muted-foreground">
                {new Date(report.date).toLocaleDateString(INTL_LOCALE[locale], { month: "long", day: "numeric", year: "numeric" })}
              </p>
            </div>
            <Button variant="outline" size="sm" className="gap-2" onClick={() => printReport(reportRef.current)}>
              <Download className="w-4 h-4" />
              {t("patientRecords.printPdf")}
            </Button>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-wrap gap-4 mb-6">
          ...
          <Badge variant="outline" className="gap-1">
            ...
            {new Date(report.date).toLocaleDateString(INTL_LOCALE[locale])}
          </Badge>
          <Badge variant="secondary">{RECORD_STATUS_KEY[report.status] ? t(RECORD_STATUS_KEY[report.status]) : report.status}</Badge>
        </div>
          ...
          <CardContent className="pt-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-foreground">{t("patientRecords.aiSimplificationTitle")}</h3>
                  <p className="text-sm text-muted-foreground">
                    {t("patientRecords.aiSimplificationDesc")}
                  </p>
                </div>
              </div>
              <Button className="gap-2 shrink-0" onClick={handleSimplify} disabled={isSimplifying}>
                <Sparkles className="w-4 h-4" />
                {isSimplifying ? t("patientRecords.simplifying") : simplified ? t("patientRecords.regenerate") : t("patientRecords.simplifyWithAi")}
              </Button>
            </div>
