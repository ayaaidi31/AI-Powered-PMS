  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
        ...
        >
          ...
          <div className="p-4 border-b border-border bg-primary text-primary-foreground">
            <div className="flex items-center justify-between">
              <h3 className="font-semibold flex items-center gap-2">
                <MessageCircle className="w-4 h-4" /> {t("aiChat.faqTitle")}
              </h3>
              <button onClick={() => setOpen(false)} className="opacity-80 hover:opacity-100" aria-label={t("aiChat.close")}>
                <X className="w-4 h-4" />
              </button>
            </div>
            <p className="text-xs opacity-90 mt-1">{t("aiChat.faqSubtitle")}</p>
          </div>
          ...
          <div className="border-t border-border bg-card">
            <p className="px-3 pt-2 text-[10px] text-muted-foreground flex items-center gap-1">
              <Sparkles className="w-3 h-3 shrink-0" /> {t("aiChat.faqDisclaimer")}
            </p>
            <form onSubmit={(e) => { e.preventDefault(); send(input) }} className="p-3 pt-2 flex gap-2">
              <input
                value={input}
                onChange={(e) => setInput(e.target.value)}
                placeholder={t("aiChat.faqPlaceholder")}
              ...
              />
              <Button type="submit" size="icon" disabled={isPending || !input.trim()} aria-label={t("aiChat.send")}>
      ...
      <Button onClick={() => setOpen(!open)} className="rounded-full w-14 h-14 shadow-lg">
        ...
        <span className="sr-only">{t("aiChat.openAssistant")}</span>
      </Button>
    </div>
  )
}
