  ...
  return (
    <div ref={ref} className="report-print w-full min-w-0 max-w-full bg-white text-neutral-900 font-serif rounded-lg border border-neutral-200 shadow-sm p-5 sm:p-8 lg:p-10 leading-relaxed">
      ...
      <div className="flex flex-wrap justify-between gap-4 mt-6">
        <div>
          <p className="text-[10px] uppercase tracking-widest text-neutral-500">Patient / Patientin</p>
          <p className="font-semibold">{patientName}</p>
          {patientDob && <p className="text-xs text-neutral-600">geb. {fmtDate(patientDob)}</p>}
        </div>
        <div className="text-right">
          <p className="text-[10px] uppercase tracking-widest text-neutral-500">Datum</p>
          <p className="font-semibold">{fmtDate(date)}</p>
        </div>
      </div>

      {/* Title */}
      <h2 className="text-lg font-bold mt-8 mb-4 text-center tracking-[0.2em]">ARZTBERICHT</h2>
      ...
      <div className="[&_h4]:text-neutral-900 [&_p]:text-neutral-800">
        {body ? (
          ...
          <div className="space-y-3 text-sm">
            {diagnosis && <p><span className="font-semibold">Diagnose: </span>{diagnosis}</p>}
      ...
      {prescriptions && prescriptions.length > 0 && (
        <div className="mt-6">
          <p className="font-semibold mb-1">Verordnungen</p>
      ...
      <div className="mt-12 flex justify-end">
        <div className="text-center">
          <p className="text-xs text-neutral-600 mb-8 text-left">{CLINIC.city}, den {fmtDate(date)}</p>
